...
export default function AddTaskModal({
  ...
  return (
    ...
    >
      <div
        ref={dialogRef}
        className="w-full max-w-md mx-4 sm:mx-auto rounded-2xl bg-white dark:bg-gray-900 p-4 sm:p-6 shadow-xl"
      >
        ...
        <div className="mb-4 flex rounded-lg border border-gray-200 dark:border-gray-700 p-0.5">
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-gray-600 dark:text-gray-400">{t("add_task_label_title")}</label>
            <input
              autoFocus
              required
              value={title}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-gray-600 dark:text-gray-400">{t("add_task_label_date")}</label>
                <button
                  type="button"
          ...
          {showCreateForm && (
            <div className="flex flex-col gap-3 rounded-lg border border-gray-200 dark:border-gray-700 p-3">
              <p className="text-xs font-medium text-gray-700 dark:text-gray-300">{t("add_task_new_category")}</p>
              <input
                autoFocus
                value={newName}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-lg px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
                >
                  {t("cancel")}
                </button>
                <button
          ...
          {hasDate && !isEvent && (
            <div className="flex flex-col gap-1">
              <label className="flex items-center gap-2 cursor-pointer text-xs font-medium text-gray-600 dark:text-gray-400">
                <input
                  type="checkbox"
                  checked={isRecurring}
          ...
          {isRecurring && (
            <div className="flex flex-col gap-3 rounded-lg border border-gray-200 dark:border-gray-700 p-3">
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-600 dark:text-gray-400 shrink-0">{t("add_task_every")}</span>
                <input
                  type="number"
                  min={1}
                  ...
                  className="w-14 rounded-lg border border-gray-200 dark:border-gray-700 px-2 py-1.5 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary-light"
                />
                <select
                  value={recurringType}
                  onChange={(e) =>
                  ...
                  className="rounded-lg border border-gray-200 dark:border-gray-700 px-2 py-1.5 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary-light"
                >
              ...
              {endCondition === "date" ? (
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-20 rounded-lg border border-gray-200 dark:border-gray-700 px-2 py-1.5 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary-light"
                  />
                  <span className="text-xs text-gray-500 dark:text-gray-400">{t("add_task_times")}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-gray-600 dark:text-gray-400">{t("add_task_label_notes")}</label>
            <textarea
              value={notes}
