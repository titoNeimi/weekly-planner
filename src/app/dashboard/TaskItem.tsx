...
export default function TaskItem({
  ...
  return (
    <div className={`group rounded-lg border px-3 py-2 transition ${
      task.done ? "border-gray-100 bg-gray-50" : "border-gray-100 bg-white hover:border-gray-200 hover:shadow-sm"
    }`}>
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          {task.category !== "General" && (
            <span className={`mt-1.5 inline-block rounded-full px-2 py-0.5 text-[10px] font-medium ${categoryColor(task.category)}`}>
              {task.category}
            </span>
          )}
        </div>
        <button
          ...
          className="invisible flex-shrink-0 text-gray-300 hover:text-red-400 group-hover:visible text-sm leading-none"
        >
          ×
        </button>
      </div>
